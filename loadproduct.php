...
$cid=$_POST['catname'];
$q1="select distinct(p.product_name) from product p, stock s where p.pid=s.pid and s.avl_quantity<>0 and s.dml_status<>2";
$q2="select distinct(p.product_name) from product p,stock s,category c where 
p.pid=s.pid and p.cid=c.cid and s.avl_quantity<>0 and c.cid=$cid and s.dml_status<>2";
